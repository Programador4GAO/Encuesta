...
// Definición de colores basada en la Paleta SIAO
const COLORS = {
  primary: '#003781',
  primaryDark: '#00245D',
  primaryHover: '#022859',
...
const ModoGerente = () => {
  ...
  useEffect(() => {
    ...
    if (user.nivel_base !== 3) {
      alert('Acceso denegado. Solo gerentes/directores (nivel 3) pueden acceder a este panel.');
      navigate('/login');
      return;
    }
  ...
  const cargarEstadisticas = async () => {
    ...
    try {
      const response = await fetch(`${API_URL}/encuestas/gerente/estadisticas/`, {
        credentials: 'include'
      });
      
      const data = await response.json();
      console.log('[Gerente] Estadísticas:', data);
      ...
      if (data.success) {
        ...
        console.error('[Gerente] Error:', data.message);
      }
    } catch (error) {
      console.error('[Gerente] Error cargando estadísticas:', error);
    } finally {
  ...
  const getNivelBadge = (nivel) => {
    if (nivel === 1) {
      return (
        <span className="px-2 py-1 text-xs font-semibold rounded-full text-white" style={{ backgroundColor: COLORS.nivel1 }}>
          Empleado
        </span>
      );
    } else if (nivel === 2) {
      return (
        <span className="px-2 py-1 text-xs font-semibold rounded-full text-white" style={{ backgroundColor: COLORS.nivel2 }}>
          Supervisor
        </span>
      );
    }
    ...
    <div className="space-y-8">
      ...
      >
        <div>
          ...
          <p className="text-lg" style={{ color: COLORS.textMain }}>
            Panel de Gerencia - Encuestas Mensuales
          </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="rounded-2xl shadow-lg p-6 text-white" 
          ...
          <div className="text-xs mt-1 opacity-75">Empleados + Supervisores</div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div 
          ...
          style={{ 
            ...
            borderLeft: `4px solid ${COLORS.nivel1}`
          }}
        >
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-3xl font-bold" style={{ color: COLORS.nivel1 }}>
              {estadisticas.nivel_1.promedio_general}
            </span>
          </div>
          <div className="grid grid-cols-3 gap-4 text-center">
            <div>
              <div className="text-2xl font-bold" style={{ color: COLORS.textTitle }}>{estadisticas.nivel_1.total}</div>
              <div className="text-xs" style={{ color: COLORS.textMain }}>Total</div>
            </div>
            <div>
              <div className="text-2xl font-bold" style={{ color: COLORS.success }}>{estadisticas.nivel_1.respondieron}</div>
              <div className="text-xs" style={{ color: COLORS.textMain }}>Respondieron</div>
            </div>
            <div>
              <div className="text-2xl font-bold" style={{ color: COLORS.error }}>{estadisticas.nivel_1.pendientes}</div>
              <div className="text-xs" style={{ color: COLORS.textMain }}>Pendientes</div>
            </div>
          </div>
          <div className="mt-4 pt-4 border-t" style={{ borderColor: COLORS.border }}>
            <div className="flex justify-between text-sm" style={{ color: COLORS.textMain }}>
              <span>Q1: <strong style={{ color: COLORS.primary }}>{estadisticas.nivel_1.promedio_q1}</strong></span>
              <span>Q2: <strong style={{ color: COLORS.primary }}>{estadisticas.nivel_1.promedio_q2}</strong></span>
              <span>Q3: <strong style={{ color: COLORS.primary }}>{estadisticas.nivel_1.promedio_q3}</strong></span>
            </div>
          </div>
        </div>
        ...
        <div 
          ...
          style={{ 
            boxShadow: `0 10px 15px -3px ${COLORS.shadow}`,
            borderLeft: `4px solid ${COLORS.nivel2}`
          }}
        >
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-xl font-bold flex items-center gap-2" style={{ color: COLORS.primary }}>
            ...
            <span className="text-3xl font-bold" style={{ color: COLORS.nivel2 }}>
              {estadisticas.nivel_2.promedio_general}
            </span>
          </div>
          <div className="grid grid-cols-3 gap-4 text-center">
            <div>
              <div className="text-2xl font-bold" style={{ color: COLORS.textTitle }}>{estadisticas.nivel_2.total}</div>
              <div className="text-xs" style={{ color: COLORS.textMain }}>Total</div>
            </div>
            <div>
              <div className="text-2xl font-bold" style={{ color: COLORS.success }}>{estadisticas.nivel_2.respondieron}</div>
              <div className="text-xs" style={{ color: COLORS.textMain }}>Respondieron</div>
            </div>
            <div>
              <div className="text-2xl font-bold" style={{ color: COLORS.error }}>{estadisticas.nivel_2.pendientes}</div>
              <div className="text-xs" style={{ color: COLORS.textMain }}>Pendientes</div>
            </div>
          </div>
          <div className="mt-4 pt-4 border-t" style={{ borderColor: COLORS.border }}>
            <div className="flex justify-between text-sm" style={{ color: COLORS.textMain }}>
              <span>Q1: <strong style={{ color: COLORS.primary }}>{estadisticas.nivel_2.promedio_q1}</strong></span>
              <span>Q2: <strong style={{ color: COLORS.primary }}>{estadisticas.nivel_2.promedio_q2}</strong></span>
              <span>Q3: <strong style={{ color: COLORS.primary }}>{estadisticas.nivel_2.promedio_q3}</strong></span>
            </div>
          </div>
        </div>
      </div>
  ...
  const renderStatistics = () => (
    <div className="space-y-8">
      ...
      <div className="bg-white rounded-2xl p-8" style={{ boxShadow: `0 10px 15px -3px ${COLORS.shadow}` }}>
        <h3 className="text-2xl font-bold mb-6" style={{ color: COLORS.primary }}>Promedios Generales (Todos los niveles)</h3>
      ...
      {/* Comparativa de Promedios por Nivel */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* Nivel 1 */}
        <div 
          className="bg-white rounded-2xl p-6"
          style={{ 
            boxShadow: `0 10px 15px -3px ${COLORS.shadow}`,
            borderTop: `4px solid ${COLORS.nivel1}`
          }}
        >
          ...
          <div className="space-y-3">
            <div className="flex justify-between" style={{ color: COLORS.textMain }}>
              <span>Q1 - Expectativas:</span>
              <span className="font-bold" style={{ color: COLORS.primary }}>{estadisticas.nivel_1.promedio_q1}</span>
            </div>
            <div className="flex justify-between" style={{ color: COLORS.textMain }}>
              <span>Q2 - Temas:</span>
              <span className="font-bold" style={{ color: COLORS.primary }}>{estadisticas.nivel_1.promedio_q2}</span>
            </div>
            <div className="flex justify-between" style={{ color: COLORS.textMain }}>
              <span>Q3 - Dinámica:</span>
              <span className="font-bold" style={{ color: COLORS.primary }}>{estadisticas.nivel_1.promedio_q3}</span>
            </div>
          </div>
        ...
        <div 
          ...
          style={{ 
            boxShadow: `0 10px 15px -3px ${COLORS.shadow}`,
            borderTop: `4px solid ${COLORS.nivel2}`
          }}
        >
          <h3 className="text-xl font-bold mb-4 flex items-center gap-2" style={{ color: COLORS.primary }}>
          ...
          <div className="space-y-3">
            <div className="flex justify-between" style={{ color: COLORS.textMain }}>
              <span>Q1 - Expectativas:</span>
              <span className="font-bold" style={{ color: COLORS.primary }}>{estadisticas.nivel_2.promedio_q1}</span>
            </div>
            <div className="flex justify-between" style={{ color: COLORS.textMain }}>
              <span>Q2 - Temas:</span>
              <span className="font-bold" style={{ color: COLORS.primary }}>{estadisticas.nivel_2.promedio_q2}</span>
            </div>
            <div className="flex justify-between" style={{ color: COLORS.textMain }}>
              <span>Q3 - Dinámica:</span>
              <span className="font-bold" style={{ color: COLORS.primary }}>{estadisticas.nivel_2.promedio_q3}</span>
            </div>
          </div>
          <div className="mt-4 pt-4 border-t text-center text-sm" style={{ borderColor: COLORS.border, color: COLORS.textMain }}>
          ...
          <table className="w-full">
            <thead style={{ backgroundColor: COLORS.primary }}>
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-white uppercase tracking-wider">Puesto</th>
                <th className="px-6 py-3 text-center text-xs font-medium text-white uppercase tracking-wider">Nivel</th>
                <th className="px-6 py-3 text-center text-xs font-medium text-white uppercase tracking-wider">Q1</th>
                <th className="px-6 py-3 text-center text-xs font-medium text-white uppercase tracking-wider">Q2</th>
                <th className="px-6 py-3 text-center text-xs font-medium text-white uppercase tracking-wider">Q3</th>
                ...
                estadisticas.usuarios_respondieron.map((usuario) => (
                  <tr key={usuario.id} className="hover:bg-[#f5f8fc]">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-center">
                      {getNivelBadge(usuario.nivel_base)}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-center">
                      <span className="px-3 py-1 text-sm font-semibold text-white rounded-full" style={{ backgroundColor: COLORS.primaryDark }}>{usuario.q1}</span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-center">
                      <span className="px-3 py-1 text-sm font-semibold text-white rounded-full" style={{ backgroundColor: COLORS.primaryDark }}>{usuario.q2}</span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-center">
                      <span className="px-3 py-1 text-sm font-semibold text-white rounded-full" style={{ backgroundColor: COLORS.primaryDark }}>{usuario.q3}</span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-center">
                      <span className="px-3 py-1 text-sm font-bold text-white rounded-full" style={{ backgroundColor: '#0070C1' }}>{usuario.promedio}</span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-center" style={{ color: COLORS.textMain }}>{usuario.fecha}</td>
                  </tr>
                ))
              )}
            </tbody>
