...
// Definición de colores basada en la Paleta SIAO [cite: 1, 2, 3, 4]
const COLORS = {
  primary: '#003781',
  primaryDark: '#00245D',
  primaryHover: '#022859',
  ...
  textTitle: '#1f2937',
  success: '#023373',
  error: '#dc2626', // Usado para pendientes/alertas
  info: '#021223',
  shadow: '#bec3c9',
  border: '#e5e7eb'
};
...
const SupervisorDashboard = () => {
  ...
  useEffect(() => {
    ...
    // Verificar nivel_base = 2 (Supervisor)
    if (user.nivel_base !== 2) {
      alert('Acceso denegado. Solo supervisores (nivel 2) pueden acceder a este panel.');
      navigate('/login');
      return;
    }
  ...
  const cargarEstadisticas = async () => {
    ...
    try {
      const response = await fetch(`${API_URL}/encuestas/supervisor/estadisticas/`, {
        credentials: 'include'
      });
      
      const data = await response.json();
      console.log('[Supervisor] Estadísticas:', data);
      ...
      if (data.success) {
        ...
        console.error('[Supervisor] Error:', data.message);
      }
    } catch (error) {
      console.error('[Supervisor] Error cargando estadísticas:', error);
    } finally {
  ...
  const renderHome = () => (
    <div className="space-y-8">
      ...
      <div 
        ...
        style={{ boxShadow: `0 10px 15px -3px ${COLORS.shadow}` }} // Sombra personalizada [cite: 4]
      >
        <div>
          ...
          <p className="text-lg" style={{ color: COLORS.textMain }}>
            Panel de Supervisor - Encuestas Mensuales
          </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {/* Card: Total Empleados - Gradiente Azul Principal */}
        <div className="rounded-2xl shadow-lg p-6 text-white" 
          ...
          <div className="text-sm font-medium opacity-90">Total Empleados</div>
          <div className="text-4xl font-bold mt-2">{estadisticas.total_empleados}</div>
          <div className="text-xs mt-1 opacity-75">En tu departamento</div>
        </div>
        
        {/* Card: Respondieron - Verde Éxito  */}
        <div className="rounded-2xl shadow-lg p-6 text-white"
        ...
        {/* Card: Pendientes - Rojo Error (Paleta: #dc2626)  */}
        <div className="rounded-2xl shadow-lg p-6 text-white"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        >
          ...
          <p className="text-center" style={{ color: COLORS.textMain }}>
            Consulta respuestas y promedios de tu equipo
          </p>
        </div>
  ...
  const renderStatistics = () => (
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between bg-white rounded-2xl p-6" style={{ boxShadow: `0 10px 15px -3px ${COLORS.shadow}` }}>
        <div>
          <h2 className="text-3xl font-bold" style={{ color: COLORS.primary }}>Estadísticas del Equipo</h2>
      ...
      <div className="bg-white rounded-2xl p-8" style={{ boxShadow: `0 10px 15px -3px ${COLORS.shadow}` }}>
        <h3 className="text-2xl font-bold mb-6" style={{ color: COLORS.primary }}>Promedios por Pregunta</h3>
    ...
    <div key={idx}>
      ...
      <div className="bg-white rounded-2xl p-8" style={{ boxShadow: `0 10px 15px -3px ${COLORS.shadow}` }}>
        <h3 className="text-2xl font-bold mb-6 flex items-center" style={{ color: COLORS.primary }}>
          ...
          Empleados que Han Respondido ({estadisticas.respondieron})
        </h3>
        
        <div className="overflow-x-auto rounded-lg border" style={{ borderColor: COLORS.border }}>
          <table className="w-full">
            {/* Header de Tabla en Azul Corporativo  */}
            <thead style={{ backgroundColor: COLORS.primary }}>
              <tr>
                {['Nombre', 'Departamento', 'Q1', 'Q2', 'Q3', 'Promedio', 'Fecha'].map((h, i) => (
                  <th key={i} className={`px-6 py-3 text-xs font-medium text-white uppercase tracking-wider ${i > 1 ? 'text-center' : 'text-left'}`}>
                    {h}
                  </th>
                ))}
              </tr>
              ...
              {estadisticas.usuarios_respondieron.length === 0 ? (
                ...
                estadisticas.usuarios_respondieron.map((usuario) => (
                  <tr key={usuario.id} className="hover:bg-[#f5f8fc]">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm" style={{ color: COLORS.textMain }}>{usuario.departamento}</td>
              ...
              {estadisticas.usuarios_pendientes.length === 0 ? (
                <tr>
                  <td colSpan="3" className="px-6 py-12 text-center text-gray-500">
                    ¡Todos han completado la encuesta!
                  </td>
                </tr>
              ) : (
